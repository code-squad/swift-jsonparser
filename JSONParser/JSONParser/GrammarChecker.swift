...
struct GrammarChecker {
    ...
    static let testSpecial = "\\\\\\\""
    /// 입력받은 문자열이 "문자열" 형태인지 체크
    static func checktestSpecial(letter: String) -> Bool {
        // 입력받은 문자열을 문자열 정규식에 통과시킨다
        let regexedLetter = extractRegexed(regexTry: GrammarChecker.testSpecial, originLetter: letter)
