...
struct MyScanner: Scanner {
    ...
    init(string: String, context: Context = Context.Value) {
        self.string = string
        self.context = context
        self.startIndex = string.index(before: string.startIndex)
        self.curser = string.startIndex
    }
    
    func hasNext() -> Bool {
        let nextIndex = self.string.index(after:self.curser)
        return nextIndex > self.string.endIndex
    }
    
    mutating func next() throws -> String {
        let string = String(self.string[self.startIndex...self.curser])
        self.startIndex = self.curser
        return string
    }
    
    mutating func scan() throws -> Character {
        guard self.hasNext() else { throw Exception.EOL }
        self.curser = self.string.index(after: self.curser)
        return self.string[self.curser]
    }
    
    func isEnd(character: Character) -> Bool {
        guard let context = Context.init(rawValue: character) else {
            return
        }
        return
    }
